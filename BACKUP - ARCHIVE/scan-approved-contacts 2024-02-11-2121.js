...
/**
 * Filter to orgs that HAVE contacts but are missing phone numbers
...
async function filterOrgsWithMissingPhones(orgs) {
    ...
    for (const org of orgs) {
        try {
            ...
            // Check if ANY contact is missing a phone number
            const missingPhones = contacts.filter(c => !c.phone || c.phone.trim() === '');
            
            if (missingPhones.length > 0) {
                needsPhones.push({
                    ...org,
                    _totalContacts: contacts.length,
                    _contactsMissingPhone: missingPhones.length
                });
            }
        } catch (error) {
            // Skip on error
...
async function main() {
    ...
    if (options.backfillPhones) {
        ...
        console.log(`   📊 ${needsPhones.length} org(s) have contacts missing phone numbers`);
        console.log(`   ⏭️ ${allApproved.length - needsPhones.length} org(s) already have phones or have 0 contacts`);
        
        if (needsPhones.length === 0) {
            console.log('   ✅ All orgs with contacts already have phone numbers. Nothing to do.');
            console.log('');
            return;
        }
    ...
    for (let i = 0; i < orgsToScan.length; i++) {
        ...
        try {
            const scanOptions = {
                ...
                scanType: options.backfillPhones ? 'phone-backfill' : 'scheduled'
            };
