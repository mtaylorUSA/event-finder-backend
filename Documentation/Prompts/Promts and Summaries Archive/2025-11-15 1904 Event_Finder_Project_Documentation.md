...
- **Always start with the app name**: Every instruction begins with the application in CAPS (e.g., "FIREFOX:", "POWERSHELL:", "FILE EXPLORER:")
- **Complete copy/paste code**: Provide full code blocks, never line-by-line edits or partial snippets
- **One step at a time**: Give single commands/actions, wait for confirmation before proceeding
- **Default to free/existing tools**: Always use free options and existing subscriptions unless there's a significant performance/difficulty downside
- **Clear success indicators**: Explicitly state "you should see X" or "this means it worked" for each step

### Troubleshooting & Debugging Approach
- **Diagnostic first**: When something fails, guide through checking specific things (Console tab, Network tab, file size, error messages) before jumping to solutions
- **Request exact error text**: Ask to copy/paste actual error messages rather than paraphrasing
- **Incremental testing**: Test one thing at a time, confirm it works, then move to the next step
- **Visual confirmation**: Provide clear descriptions of what success looks like at each step
- **Explain the "why"**: When debugging, briefly explain what we're checking for and why it helps diagnose the issue

### Communication Style
- **Ask clarifying questions**: User will ask for clarification when instructions aren't clear rather than guessing
- **Provide context**: User shares specific details (error messages, file sizes, what's visible) to help diagnose issues
- **Confirm progress**: User responds with "it worked!", "perfect!", or describes what they see
- **Detail-oriented**: User notices small discrepancies (like wrong file sizes) and reports them
- **Proactive documentation**: User thinks ahead about creating references for future use

### Technical Comfort Level
- **Command line capable**: Comfortable with terminal/PowerShell when given exact commands
- **Prefers GUI when available**: Likes using File Explorer, Firefox UI, visual tools over command line for file operations when both options exist
- **Methodical approach**: Works through steps in order, doesn't skip ahead
- **Organized**: Maintains credentials in BitWarden, keeps organized folder structure
- **Persistent problem-solver**: Keeps trying different approaches when something doesn't work

### Learning & Documentation Preferences
- **Reference materials**: Appreciates having documentation to upload in future conversations (like the PocketBase field types reference)
- **Comprehensive guides**: Values detailed documentation that can be reused
- **Visual examples**: Benefits from seeing examples of success/failure states
- **Context preservation**: Proactively creates documentation files for future Claude sessions

### Design Preferences (Product-Specific)
- **Approve organizations, not events**: Workflow focused on org approval; events are auto-scraped
- **Events auto-scraped after org approval**: No manual event entry for approved orgs
- **Show 20 org suggestions per discovery run**: AI discovery returns 20 suggestions
- **Allow approving 1-n organizations at a time**: Flexible batch approval
- **3-second visual feedback before removing cards**: Clear confirmation of approve/reject actions
- **Alphabetically sorted lists**: Organizations displayed in alphabetical order

### Error Handling Preferences
- **Don't hide failures**: Show errors clearly so they can be diagnosed
- **Explain what went wrong**: Brief explanation of why something failed
- **Offer alternatives**: When one approach fails, suggest different methods
- **Conservative file operations**: Confirm before overwriting or deleting files

### Tool Preferences
- **Browser**: Firefox (primary)
- **Terminal**: Windows PowerShell
- **File Management**: Windows File Explorer
- **Credentials**: BitWarden for secure storage
- **Version Control**: Git/GitHub (with guidance)
