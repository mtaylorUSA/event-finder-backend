...
│         │                   │                   │                          │
│         ▼                   ▼                   ▼                          │
│   - Dedupe check       - Approved or       - POC moved to                  │
│   - TOU flagged        - Rejected            `contacts`                    │
│     if issue                                - Permission tracked           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
4. For each suggestion, AI gathers **publicly available info only** (no event scraping):
   ...
   - POC / contact info (temporary — will move to `contacts` later)
...
- ⚠️ If TOU may prohibit scraping → Set `tou_flag = true` and add details to `tou_notes`
...
| Decision | Action |
|----------|--------|
| ✅ Approved | Set `status = Mission Approved`, proceed to Permission stage |
| ❌ Rejected | Move to `rejected_organizations` with `rejection_reason = Mission Mismatch` |
...
1. Open PocketBase Admin → `contacts` collection
2. Click "+ New record"
3. Fill in fields:
   - `name` — POC name from suggested org
   - `email` — POC email from suggested org
   - `title` — POC title from suggested org
   - `organization` — Select the approved organization
   - `contact_type` — Select appropriate type (e.g., `Legal/Permissions`)
4. Click "Save"
...
- `Events` — Events coordinator, calendar manager
- `Legal/Permissions` — Who approves scraping
- `Media/PR` — Press contacts
- `Other` — Catch-all
...
| Decision | Action |
|----------|--------|
| 🟢 Granted | Set `scraping_enabled = true`, add `events_url` |
| 🔴 Rejected | Move to `rejected_organizations` with `rejection_reason = Permission Denied` |
...
|------------------|-----------------|--------|
| `Duplicate` | AI suggestion (Stage 1) | Skip — don't save to `suggested_organizations` |
...
│   - poc_name            │  ← AI gathers (temporary)
│   - poc_email           │
│   - poc_title           │
└───────────┬─────────────┘
            │ Admin approves & creates contact
            ▼
┌─────────────────────────┐
│       contacts          │
...
│   - email               │
│   - title               │
│   - organization (rel)  │  ← Links to organizations
│   - contact_type        │
│   - last_verified       │
└─────────────────────────┘
```
...
| Create contact records | PocketBase Admin → `contacts` |
| Track permission status | PocketBase Admin → `organizations` |
| Upload permission docs | PocketBase Admin → `organizations` → `permission_document` |
| Send permission requests | Manual email to contacts |
| Enable/disable scrapers | PocketBase → `organizations` → `scraping_enabled` |
| View scraped events | PocketBase Admin → `events` |
| View event embeddings | PocketBase Admin → `event_embeddings` |

---

## Key Database Fields Reference

### `suggested_organizations` — Status Values

```
Pending, Mission Approved, Mission Rejected
```

### `rejected_organizations` — Rejection Reasons

```
Mission Mismatch, Permission Denied, TOU Prohibits, Duplicate, Other
```

### `organizations` — Permission Status Values

```
Not Yet Requested, Pending, Granted, Rejected
```

### `organizations` — Permission Requested By
