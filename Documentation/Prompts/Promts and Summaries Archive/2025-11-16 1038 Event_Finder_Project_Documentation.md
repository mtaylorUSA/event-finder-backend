...
- **Format: APP → LOCATION/TOOL: Action** - Every instruction must start with the application name, followed by the specific location or tool within that app, then the action
  - ✅ Good: "FIREFOX → GitHub repo page: Click on the **Actions** tab"
  - ✅ Good: "POWERSHELL → Event Finder folder: Type `node proxy-server.js`"
  - ✅ Good: "FILE EXPLORER → Downloads folder: Move the file to Event Finder"
  - ❌ Bad: "FIREFOX: Click on the Actions tab" (missing location context)
  - ❌ Bad: "Click on the Actions tab" (missing app entirely)

**Other Preferences:**
- Complete copy/paste code (no line-by-line edits)
